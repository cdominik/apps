...
Formation requires $\geq 30$ levitated particles and spread $s \geq 0.20$.
A revolution counter accumulates at $\Delta t / T$; first aggregate forms
...
7 levitated aggregates held for 3 revolutions, spread $\geq 0.30$.
...
\texttt{Space} freezes all physics, drum rotation, and motor audio while
leaving the canvas rendering active.
...
Live KDE: yellow (floating), green (levitated), amber histogram (aggregates).
...
\begin{enumerate}
  ...
  \item \textbf{Live} (green) --- a KDE-shaded band, 4 drum-units tall,
    ...
    Colour at position $x$ is drawn as a column of height equal to the strip
    half-height, with alpha $\propto \mathrm{KDE}(x) \times 0.65$.
\end{enumerate}

The live band makes the drum's aerodynamic selection effect directly
visible: the band narrows as the drum retains only particles near the
optimal orbit radius, and an aggregate sub-band (if present) shows any
Stokes-number offset relative to the monomers.
...
Blade extends from the slot marker inward, sweeping $240°$ ($4\pi/3$) in two
...
aggregates, placing each copy $90°$ ahead in its orbit:
\begin{align}
  x' &= \xc - y, \quad y' = x - \xc, \\
...
\begin{longtable}{@{}p{2cm}lp{9cm}@{}}
  ...
  \btnicon{winSpread} & \textbf{$v_t$ spread} &
    $s \in \{0,0.01,...,0.50\}$. Min 0.20 for aggregates, 0.30 for pebbles.\\
...
\begin{longtable}{@{}p{2cm}lp{9cm}@{}}
  ...
  \btnicon{btnLaser}    & \textbf{Lidar}       & Lidar mode; laser at $\omg_{\mathrm{laser}} = 2\pi/1.2\,\mathrm{rad\,s^{-1}}$. \\
  ...
  \btnicon{btnTheme}    & \textbf{Lighting}    & Dark / light lab background. \\
  ...
  \btnicon{btnManual}   & \textbf{Info}        & Open user manual. \\
  \btnicon{btnReset}    & \textbf{Reset}       & Clear all objects; reset to clean state.
                                                 Also disables zoom mode if active. \\
  \bottomrule
\end{longtable}
...
\begin{longtable}{@{}p{1.5cm}lp{9.5cm}@{}}
  ...
  \btnicon{btnVtDist} & \textbf{$v_t$ distribution} &
    Live KDE: levitated (green), floating (yellow), aggregate histogram (amber). \\
...
\begin{longtable}{@{}p{1.5cm}lp{9.5cm}@{}}
  ...
  \btnicon{btnProcDouble} & \textbf{Widen slice}  & Duplicate levitated objects $90°$ ahead. \\
...
\begin{longtable}{@{}p{1.5cm}lp{9.5cm}@{}}
  ...
  \btnicon{btnSysAutoOmega}  & \textbf{Auto-omega}     & Off / auto / launch. \\
  \btnicon{btnCollect}       & \textbf{Collect}        & Arm collection tray. \\
  $\blacktriangleleft$       & \textbf{Speed down}     & Reduce simulation speed one gear. \\
  \btnicon{btnSpeedReset}    & \textbf{Speed}          &
    Current gear label (letter-spacing tightened for sub-unity speeds);
    press to reset to $1\times$. \\
  ...
  \btnicon{btnProcSlowMo}    & \textbf{Slow motion}    &
    ...
    Long press ($>2\,\mathrm{s}$): Fast Chain mode. \\
...
\begin{longtable}{@{}lp{11cm}@{}}
  ...
  \texttt{Space} &
    ...
    audio freeze; canvas continues to render the frozen state. \\
  \texttt{?} &
    Toggle the hint system (see §\ref{sec:hints}). When on, contextual
...
Pressing \texttt{?} toggles \texttt{window.hintsOn}. Messages appear as
italic brass-coloured text on a dark rounded backdrop near the top of the drum
interior (drum coordinate $y \approx 78$), drawn on \texttt{ctxOv} each frame
independently of strobe mode. The opacity follows a three-phase envelope:
...
exclusive by their Stokes-kick condition, so only one fires at a time.
This ensures the message is physically accurate: without the kick the orbit
centre does not shift; with it, the Stokes number increase is the cause.
...
\begin{longtable}{@{}lrl@{}}
  ...
  \texttt{tray.totalAngle}       & $4\pi/3$ & Total insertion sweep ($240°$) \\
