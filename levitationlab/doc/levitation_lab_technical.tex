...
One snapshot is recorded every $10\,\mathrm{s}$ of wall-clock time while the
simulation is running and not paused. A maximum of 20 snapshots are retained
(a rolling buffer discarding the oldest), giving up to $200\,\mathrm{s}$ of
history at all particle counts. Recording is skipped at particle counts above
approximately 10\,000 where the per-snapshot memory cost ($\sim$12\,MB) would
become significant; the cap ensures total buffer memory stays well below
$25\,\mathrm{MB}$ in all practical configurations.
...
\texttt{tray}. Particle trails and diagnostic-target flags are
intentionally excluded. The heatmap accumulators are not saved and
are not restored.
...
While paused:
\begin{itemize}
  \item \textbf{Keyboard}: \texttt{←} / \texttt{→} step the cursor one
    snapshot at a time.
...
\begin{longtable}{@{}p{1.5cm}lp{9.5cm}@{}}
  ...
  \btnicon{btnProcDouble} & \textbf{Duplicate}    & Duplicate levitated objects $90°$ ahead. \\
...
\begin{longtable}{@{}p{1.5cm}lp{9.5cm}@{}}
  ...
  \btnicon{btnThemeStyle}    & \textbf{Visual theme}    &
    Cycles: default brass-and-dark $\to$ clinical \emph{modern} (green) $\to$
    \emph{Pfeiffer} red-structures variant (orange) $\to$ default.
    The same themes can be selected at startup via \texttt{?modern} and
    \texttt{?pfeiffer} URL flags. \\
...
\begin{itemize}
  ...
  \item Timeline snapshots: one per $10\,\mathrm{s}$ wall-clock time, max 20
    ...
    memory $\ll 25\,\mathrm{MB}$ at $N_P \leq 3000$; recording disabled at
    higher counts.
\end{itemize}
